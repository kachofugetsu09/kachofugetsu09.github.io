...
window.SITE_DATA = {
    ...
    "articleDetails": {
        ...
        "CS186/CS186笔记.md": {
            "title": "CS186 笔记",
            "preview": "# CS186 笔记 DBMS 架构是一个分层的软件系统，负责将用户的高级数据请求（如 SQL）转化为对物理存储设备的底层操作。它像一个精密的工厂，每一层都有明确的分工。 直接操作磁盘是极其复杂和低效的。一个分层的架构可以将复杂的问题分解，每一层专注于解决特定的问题（如查询解析、并发控制、内存管理、磁盘 I/O），从而实现关注点分离。这使得 DBMS 的设计、实现和维护变得更加模块化和高效。 一个...",
            "updateTime": "2025-07-11",
            "wordCount": 9635,
            "readingTime": 65,
