...
function loadCategoryConfig(categoryPath, categoryKey) {
  ...
  try {
    if (!fs.existsSync(readmePath)) {
      console.log(
        `📁 ${categoryKey}: 未找到 README.md，使用文件夹名作为分类名`
      );
      console.log(
        `   💡 提示: 可以创建 ${categoryKey}/README.md 来自定义分类名和图标`
      );
      console.log(
        `   📝 格式: ---\\nname: "自定义名称"\\nicon: "fas fa-code"\\n---`
      );
    ...
    if (!config) {
      console.warn(
        `${categoryKey}/README.md 中未找到有效的Front Matter配置，使用默认配置`
      );
      return defaultConfig;
    }
...
/**
 * 获取文件修改时间（优先使用Git提交时间，回退到文件系统时间）
 */
function getFileModifiedTime(filePath) {
  try {
    ...
    try {
      ...
      if (gitTime) {
        const commitDate = new Date(gitTime);
        return {
          precise: commitDate.toISOString(), // 精确时间用于排序
          display: commitDate.toISOString().split("T")[0], // 显示时间只到日期
        };
      }
    } catch (gitError) {
      console.warn(`获取Git提交时间失败 ${filePath}:`, gitError.message);
    }
    ...
    return {
      precise: mtime.toISOString(), // 精确时间用于排序
      display: mtime.toISOString().split("T")[0], // 显示时间只到日期
    };
  } catch (error) {
    console.warn(`获取文件修改时间失败 ${filePath}:`, error);
    const now = new Date();
    return {
      precise: now.toISOString(),
...
/**
 * 提取Markdown文件的标题 - 优先使用文件名
 */
function extractTitleFromMarkdown(filePath) {
  try {
    // 直接使用文件名（去掉.md扩展名）作为标题
...
function generateArticleConfig() {
  ...
  for (const [categoryKey, categoryInfo] of sortedCategories) {
    ...
    console.log(
      `分类 "${categoryInfo.name}" (${categoryKey}): 找到 ${filteredFiles.length} 个文件`
    );
    ...
    for (const relativePath of filteredFiles) {
      ...
      const articleDetails = {
        title: title,
        preview: preview,
        updateTime: updateTime.display, // 显示用日期
        wordCount: wordCount,
        readingTime: readingTime,
        relativePath: relativePath, // 添加相对路径信息
        _sortTime: updateTime.precise, // 内部排序用精确时间
      };
      ...
      console.log(
        `  - ${title} (${relativePath}) - ${wordCount}字, ${readingTime}分钟`
      );
    }
  ...
  for (const categoryKey of Object.keys(articles)) {
    articles[categoryKey].sort((a, b) => {
      const aTime = articleDetailsMap[`${categoryKey}/${a}`]._sortTime;
      const bTime = articleDetailsMap[`${categoryKey}/${b}`]._sortTime;
      return new Date(bTime) - new Date(aTime);
    });
  }

  allArticles.sort((a, b) => new Date(b._sortTime) - new Date(a._sortTime));
...
function generateSitemap(config) {
  ...
  // 添加文章页面
  for (const [categoryKey, articleList] of Object.entries(
    config.articleLists
  )) {
    for (const filename of articleList) {
      urls.push(
        `article.html?category=${categoryKey}&file=${encodeURIComponent(
          filename
        )}`
      );
    }
...
function generateReadme(config) {
  ...
  for (const [categoryKey, categoryInfo] of Object.entries(config.categories)) {
    ...
    for (const filename of articleList) {
      ...
      if (article) {
        ...
        lines.push(
          `- [${title}](${categoryKey}/${filename}) - ${updateTime} - ${wordCount}字 - ${readingTime}分钟`
        );
      }
    }
    lines.push("");
  ...
  lines.push("## 统计信息");
  lines.push("");
  const totalArticles = Object.values(config.articleLists).reduce(
