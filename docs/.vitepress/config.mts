...
export default withMermaid(defineConfig({
  ...
  themeConfig: {
    ...
    {text: '主页',link:'/main'},
{
        text: '技术分享',collapsed: true,
        items: [
          {text: 'ARIES与innodb',link: '/tech/ARIES与innodb'},
              {text: 'Kafka和RocketMQ的背压机制解析',link: '/tech/Kafka和RocketMQ的背压机制解析'},
              {text: '零拷贝技术解析',link: '/tech/零拷贝技术解析'},
        ]
      },
      {
        text: 'CS186学习',collapsed: true,
        items: [
           {text: 'CS186笔记',collapsed: true,
              items:[
      ...
      {
        text: 'JDK源码解析',collapsed: true,
        items: [
          {text: 'AQS和可重入锁', link: '/jdk/AQS和可重入锁'},
        ]
      },
      {
        text: 'Spring框架',collapsed: true,
        items: [
          {text: 'JWT双Token深度解析', link: '/spring/JWT双Token深度解析：平衡安全与效率的多端登录实践'},
          {text: 'Spring事务机制', link: '/spring/spring当中的事务是如何运作的'},
          {text: 'Kafka专题', collapsed: true,
            items: [
              {text: 'Kafka基本用法', link: '/spring/kafka/kafka基本用法'},
              {text: 'Kafka再平衡机制', link: '/spring/kafka/kafka的再平衡机制'},
              {text: 'Kafka Streams实践', link: '/spring/kafka/以点赞系统为例的kafka streams用法'}
            ]
          }
        ]
      },
       {
        text: 'redis-mini实现',collapsed: true,
        items: [
          {text: '初版渐进式哈希实现思路', link: '/redis-mini/渐进式哈希实现思路'},
